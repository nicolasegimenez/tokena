...
export default function TradePage() {
  ...
  const filteredListings = useMemo(() => {
    let listings = marketProjects.filter((listing) => {
      const searchTermMatch = listing.projectName
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const categoryMatch =
        selectedCategory === "all" ||
        listing.category.toLowerCase().replace(" ", "-") === selectedCategory;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <div
        className={`border-b bg-white dark:bg-slate-900 transition-all duration-300 ease-in-out transform ${
          showHeader
      ...
      <div className="container mx-auto px-4 py-8">
        {/* Filter Button and Active Filters Badge */}
        <div className="mb-6 flex items-center gap-3">
          <Button
            variant="outline"
            className="flex items-center gap-2 border-emerald-200/50 dark:border-emerald-800/50 hover:bg-emerald-50 dark:hover:bg-emerald-950/30"
          ...
          {(selectedCategory !== "all" || sortBy !== "price-asc") && (
            <div className="flex items-center gap-2">
              <Badge variant="secondary" className="text-xs">
                {(selectedCategory !== "all" ? 1 : 0) + (sortBy !== "price-asc" ? 1 : 0)} {language === "es" ? "Filtros activos" : "Active filters"}
              </Badge>
              <Button
                variant="ghost"
                size="sm"
        ...
        <Dialog open={showFiltersDialog} onOpenChange={setShowFiltersDialog}>
          <DialogContent className="max-w-md mx-1 max-h-[85vh] overflow-y-auto">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">
                {t("filters")}
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {language === "es" ? "Filtra y ordena los listados según tus preferencias" : "Filter and sort listings according to your preferences"}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 h-12 bg-white/80 dark:bg-slate-800/80 border-emerald-200/50 dark:border-emerald-800/50 focus:border-emerald-500 focus:ring-emerald-500/20"
                    value={searchTerm}
              ...
              {(selectedCategory !== "all" || sortBy !== "price-asc" || searchTerm !== "") && (
                <Button
                  variant="outline"
                  onClick={() => {
                    setSearchTerm("");
                  ...
                  <CardContent className="pt-6">
                    <form onSubmit={handleSellSubmit} className="grid gap-6">
                      <div className="grid gap-2">
                        ...
                        <Select
                          onValueChange={setSelectedTokenToSell}
                          value={selectedTokenToSell}
                        >
                          <SelectTrigger id="tokenToSell" className="h-11">
                            <SelectValue
                              placeholder={t("select_from_portfolio")}
                            />
                          </SelectTrigger>
                          <SelectContent>
                            {myTokens.map((token) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <p>
                                      {token.projectName} ({token.tokenSymbol})
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {token.availableToSell}{" "}
                                      {t("available_to_sell")}
                                    </p>
                                  </div>
                                </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-bold">
                                {myTokens.find(
                                  (t) => t.tokenSymbol === selectedTokenToSell
                                )?.availableToSell || 0}
                              </p>
                            </div>
                          </div>
