        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Tipo de Inversión</p>
                <p className="font-semibold">{projectData.investmentDetails.activo}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Horizonte Temporal</p>
                <p className="font-semibold">{projectData.investmentDetails.duracionCiclo}</p>
              </div>
              <div className="col-span-2">
                <p className="text-sm text-muted-foreground">Capital Objetivo (Seed Round)</p>
                <p className="font-semibold">{projectData.investmentDetails.capitalRequerido}</p>
              </div>
              <div className="col-span-2">
                <p className="text-sm text-muted-foreground">Oferta Inicial</p>
                <p className="font-semibold">{projectData.investmentDetails.capitalTokenizado}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <ol className="space-y-3">
                {projectData.modeloNegocio.map((paso, idx) => (
          ...
          <Card>
            ...
            <CardContent>
              <ul className="space-y-3">
                {projectData.estructuraTokenizacion.map((elemento, idx) => (
                  <li key={idx} className="flex gap-3">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Roadmap 2025–2028</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {projectData.cronograma.map((hito, idx) => (
                <div key={idx} className="border-l-4 border-primary pl-4 pb-3 last:pb-0">
                  <p className="font-semibold text-sm">{hito.fase}</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Ventajas de Invertir en Investoken</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-3">
                {projectData.ventajasTokenizacion.map((ventaja, idx) => (
                  <li key={idx} className="flex gap-3">
                    <span className="text-primary font-bold">✓</span>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Modalidad de Inversión</span>
                <span className="font-medium">{projectData.compliance.tokenStandard}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Estructura Legal</span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">KYC/AML Requerido</span>
                <span className="font-medium">{projectData.compliance.kycRequired ? "Sí" : "No"}</span>
              </div>
              <div>
                <h4 className="font-medium mb-2">Documentos Legales</h4>
